...
def lines_list(session, device_uuid, url):
    ...
    response = requests_retry_session(session=session).post(url, data={}, headers=headers, timeout=10)
    return response.text

def node_protocol(session, device_uuid, code, url):
    data = {"code": code}
    headers = prepare_headers(session, device_uuid)
    response = requests_retry_session(session=session).post(url, data=data, headers=headers, timeout=10)
    return response.text
...
def fetch_from_api(lines_list_url, node_protocol_url, device_uuid):
    ...
    try:
        ...
        for n in nodes:
            code = n['code']
            node_protocol_result = node_protocol(session, device_uuid, code, node_protocol_url)
